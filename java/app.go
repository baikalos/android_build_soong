...
func AutogeneratedRroModuleName(ctx android.EarlyModuleContext, moduleName, partition string) string {
	...
	product := strings.Replace(ctx.Config().Getenv("TARGET_PRODUCT"), "lineage_", "crdroid_", 1)
	overrideName := strings.Replace(strings.Replace(strings.Replace(moduleName, "lineageos", "crdroid", -1), "Lineage", "crDroid", -1), "lineage", "crdroid", -1)
