...
func (a *AndroidApp) installPath(ctx android.ModuleContext) android.InstallPath {
    ...
    ainstallApkName := a.installApkName
	if ctx.ModuleName() == "framework-res" {
        ...
        ainstallApkName = "org.mineageos.platform-res"
		installDir = "framework"
	} else if ctx.ModuleName() == "org.mineageos.platform-res" {
		installDir = "framework"
	...
	return android.PathForModuleInstall(ctx, installDir, ainstallApkName+".apk")
}
...
func (a *AndroidApp) dexBuildActions(ctx android.ModuleContext) (android.Path, android.Path, *JavaInfo) {
	...
	javaInfo := &JavaInfo{}
	if ctx.ModuleName() != "framework-res" && ctx.ModuleName() != "org.lineageos.platform-res" && ctx.ModuleName() != "org.mineageos.platform-res" {
...
func (a *AndroidApp) generateAndroidBuildActions(ctx android.ModuleContext) {
	...
	packageFile := android.PathForModuleOut(ctx, ainstallApkName+".apk")
	v4SigningRequested := Bool(a.Module.deviceProperties.V4_signature)
	var v4SignatureFile android.WritablePath = nil
	if v4SigningRequested {
		v4SignatureFile = android.PathForModuleOut(ctx, ainstallApkName+".apk.idsig")
	}
	...
	for _, split := range a.aapt.splits {
		// Sign the split APKs
		packageFile := android.PathForModuleOut(ctx, ainstallApkName+"_"+split.suffix+".apk")
		if v4SigningRequested {
			v4SignatureFile = android.PathForModuleOut(ctx, ainstallApkName+"_"+split.suffix+".apk.idsig")
		}
	...
	if shouldInstallAppPackage {
		if a.privAppAllowlist.Valid() {
			...
			allowlistInstallFilename := ainstallApkName + ".xml"
...
func AutogeneratedRroModuleName(ctx android.EarlyModuleContext, moduleName, partition string) string {
    ...
    // Replace lineage_ and auto_generated markers
    deviceName := ctx.Config().Getenv("TARGET_PRODUCT")
    cleanDevice := strings.ReplaceAll(deviceName, "lineage", "baikalos")
	overrideName := strings.Replace(strings.Replace(strings.Replace(moduleName, "lineageos", "baikalos", -1), "Lineage", "BaikalOS", -1), "lineage", "baikalos", -1)
	return fmt.Sprintf("%s__%s__auto_generated_rro_%s", overrideName, cleanDevice, partition)
}
