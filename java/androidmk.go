	...
	return []android.AndroidMkEntries{android.AndroidMkEntries{
		...
		ExtraEntries: []android.AndroidMkExtraEntriesFunc{
			func(ctx android.AndroidMkExtraEntriesContext, entries *android.AndroidMkEntries) {
				...
				if app.Name() == "framework-res" || app.Name() == "org.lineageos.platform-res" || app.Name() == "org.mineageos.platform-res" {
...
func (a *AndroidLibrary) AndroidMkEntries() []android.AndroidMkEntries {
	...
	entries.ExtraEntries = append(entries.ExtraEntries, func(ctx android.AndroidMkExtraEntriesContext, entries *android.AndroidMkEntries) {
		...
		if a.Name() == "framework-res" || a.Name() == "org.lineageos.platform-res" || a.Name() == "org.mineageos.platform-res" {
			entries.SetString("LOCAL_MODULE_PATH", "$(TARGET_OUT_JAVA_LIBRARIES)")
			// Make base_rules.mk not put framework-res in a subdirectory called
			// framework_res.
			entries.SetBoolIfTrue("LOCAL_NO_STANDARD_LIBRARIES", true)
